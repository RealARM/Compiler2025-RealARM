...
def compile_project():
    ...
    try:
        ...
        # 尝试按 UTF-8 解码，若失败则回退到系统默认编码，最后保证不抛出异常
        try:
            decoded_msg = e.stderr.decode('utf-8')
        except UnicodeDecodeError:
            try:
                # Windows 中文环境通常使用 GBK/GB2312
                decoded_msg = e.stderr.decode('gbk')
            except UnicodeDecodeError:
                # 最后降级为“replace”策略，确保不会再次抛异常
                decoded_msg = e.stderr.decode('utf-8', errors='replace')
        print_color(f"编译失败: {decoded_msg}", Colors.RED)
        return False
